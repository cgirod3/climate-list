import React from 'react';

import '../styles/components/Header.scss';

const Header = () => {
  return (
    <div className='Header'>
      <h2>The GreenTech List</h2>
      <h3 className='Header-quote'>"Your work is going to fill a large part of your life, and the only way to be truly satisfied is to do what you believe is great work."</h3>
      <h3>- Steve Jobs</h3>
      Climate Change is the most pressing issue facing the world today.
      This page is intended to be a resource to connect those interested
      in fixing the problem with companies actively working on solutions.
    </div>
  );
};

export default Header;
